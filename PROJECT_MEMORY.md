...
- 播放时间推进到第 N 帧后，`index < floor(frameFloat)` 的历史切片直接设为不可见；当前帧和未播放帧仍保留在直线时间序列中，避免已越过的切片继续从镜头前方堆叠。
...
- 根据确认后的新构图，将时间切片改为保持正面平行、沿世界 X 轴以 `LINE_STEP = 0.30` 排列的单一直线；切片不再使用 X+Z 轨迹、逐片倾斜或 S 形微动，统一 `rotationRig` 仍负责完整视角旋转。
...
- “展开切片”按钮让 166 张透明时间切片沿单一水平直线展开或收拢。
- 展开后能看到连续的 3D 直线时间序列、遮挡关系和视角变化。
- 当前帧固定在视觉中心，播放会让时间切片沿直线相对焦点移动，形成“时间穿梭”效果。
...
- 播放推进到当前帧后，`index < floor(frameFloat)` 的切片通过 `visible = false` 从渲染中退出；当前帧及其后的时间切片继续参与空间布局和透明度计算。
- 滚轮 → 相机距离目标值 → 平滑缩放主体，距离范围为 3–15.5，并将镜头焦点对准当前 `frameFloat` 的连续时间切片中心；使用更快的可打断阻尼，不修改 `frameFloat`、`yaw`、`pitch` 或 `spread`。
- `offset = index - frameFloat` → 当前帧中心锚定的直线位置；`slice.position.x = offset × 0.30 × spread`，Y/Z 保持为 0，切片自身旋转保持为 0。
- 展开按钮 → `spreadTarget`，控制切片沿 X 轴直线展开；拖拽不再改变展开状态。
- Pointer Events 拖拽超过约 8px 起拖死区后，横向增量独立累积不封顶的 `yawTarget`，纵向增量按“下拖俯视、上拖仰视”独立映射到 ±45° 范围内的 `pitchTarget`；斜向拖动同时改变两个角度，可连续绕 Y 轴 360° 旋转；播放期间同样消费拖拽增量。
- `rotationRig` 统一旋转时间切片，固定相机不随用户视角移动；拖拽时使用 14、松手时使用 8 的更快短阻尼过渡，不产生松手惯性。
...
- 播放中的历史切片不是被删除；它们只在 `index < floor(frameFloat)` 时从 Three.js 渲染树暂时隐藏，倒放回到较早帧时会重新可见。
...
- 画布横向拖拽可连续旋转 360° 及更多圈，纵向拖拽限制在 ±45°且响应更灵活；双轴反向操作不会跳变，斜向拖动仍能同时调整视角。
- 右侧“正放”“倒放”单击可以开始/暂停对应方向播放；“开头”“结尾”单击后停在相应边界，播放时当前帧之前的切片不可见。
