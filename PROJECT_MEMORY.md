...
- 同次修改将滚轮职责限定为主体缩放：向上滚轮拉近相机、向下滚轮拉远相机，使用 5.8–15.5 的距离边界和可打断阻尼，不改变时间帧或旋转角度；页面改为单屏舞台，不再用滚动驱动时间。
- 长按和旋转采用 8px 分流死区：未触发长按前移动超过死区进入双轴拖拽，播放期间不旋转；保留向下俯视、向上仰视、连续 360° 以上 yaw 和丰富环境动效。
...
- 2026-08-30：按眩晕反馈重构旋转交互。横向拖拽只累积 `yawTarget`，纵向拖拽只调整 ±28° 的 `pitchTarget`；拖拽超过约 8px 后锁定主方向，展开切片改由按钮独立控制。
...
- 长按超过约 260ms → `isPlaying` → 以 30fps 推进 `frameFloat`，松开或到达末帧暂停。
- 滚轮 → 相机距离目标值 → 平滑缩放主体，不修改 `frameFloat`、`yaw`、`pitch` 或 `spread`。
...
- 长按后从当前帧连续播放，松开立即暂停，播放到末帧后停留，再次长按可从首帧重新播放。
...
- 展开按钮不会改变当前旋转视角，长按播放和滚轮缩放不会触发自动镜头绕行。
- 展开后有明显的空间透视、遮挡和时间顺序。
- 单屏画布可用，移动端长按播放和双轴拖拽不会被浏览器默认手势抢占。
