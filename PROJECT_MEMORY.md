...
- 当前帧固定在视觉中心，播放会让时间切片沿深度轴相对焦点移动，形成“时间穿梭”效果。
...
- 所有切片均保持 `visible = true` 并参与空间布局；`frameFloat` 负责把当前帧锚定在中心以及驱动各帧的距离透明度分级，不再裁切当前帧之前的历史切片。展开态当前帧约为 `0.96`，相邻、中距离和远距离切片依次降至约 `0.16–0.30`、`0.06–0.14`、`0.02–0.06`；收拢态额外以 `Math.round(frameFloat)` 作为唯一清晰帧，其余重叠切片淡出。
- 当前帧由透明度、运行时饱和度/对比度和低强度暖粉色加法混合副层共同突出；微光按 `frameFloat` 连续焦点权重过渡，以约 `1.7s` 周期轻微呼吸，`prefers-reduced-motion` 下只保留静态焦点光。
- 滚轮 → 相机距离目标值 → 平滑缩放主体，距离范围为 3–15.5，并将镜头焦点对准当前 `frameFloat` 的连续时间切片中心；使用更快的可打断阻尼，不修改 `frameFloat`、`yaw`、`pitch` 或 `spread`。
- `offset = index - frameFloat` → 当前帧中心锚定的深度位置；`slice.position = (0, 0, -offset × 1 × spread)`，切片自身旋转保持为 0。
