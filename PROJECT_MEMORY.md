...
参考视频的重点不是复用蝴蝶素材，而是复用它的交互语法和构图：实时 3D 主体、时间切片、滚动控制时间、展开时间轴，以及可控的 3D 视角旋转。
...
- `rotationRig` 统一旋转兰花和切片，固定相机不随用户视角移动；目标状态使用短阻尼过渡，不产生松手惯性。
- 主体、切片和环境粒子仍有低幅度漂浮与呼吸动效，但滚动不再触发自动镜头绕行。
...
6. 实现实时 GLB 动画、时间切片、滚动映射、切片展开和自由视角旋转。
