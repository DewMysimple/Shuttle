...
- 播放时间推进到第 N 帧后，`index < floor(frameFloat)` 的历史切片直接设为不可见；当前帧和未播放帧仍保留在平行深度序列中，避免已越过的切片继续从镜头前方堆叠。
...
- 根据最新反馈，将时间切片从共面的 X 轴排列改为保持正面平行、沿卡片法线的世界 Z 轴以 `SLICE_DEPTH_STEP = 0.42` 等距分层；切片不再共面、相交、弯曲或逐片倾斜，统一 `rotationRig` 仍负责完整视角旋转。
...
- `offset = index - frameFloat` → 当前帧中心锚定的深度位置；`slice.position = (0, 0, -offset × 0.42 × spread)`，切片自身旋转保持为 0。
