...
- Pointer Events 拖拽超过约 8px 后锁定主方向：横向增量累积不封顶的 `yawTarget`，纵向增量映射到 ±28° 范围内的 `pitchTarget`，可连续绕 Y 轴 360° 旋转。
- `rotationRig` 统一旋转时间切片，固定相机不随用户视角移动；目标状态使用短阻尼过渡，不产生松手惯性。
- 时间切片和环境粒子仍有低幅度漂浮与呼吸动效，但滚动不再触发自动镜头绕行。
...
- 画布横向拖拽可连续旋转 360° 及更多圈，纵向拖拽限制在 ±28°，方向锁定后反向不会跳变。
- 展开按钮不会改变当前旋转视角，滚动不会触发自动镜头绕行。
- 展开后有明显的空间透视、遮挡和时间顺序。
