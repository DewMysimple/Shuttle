...
- 所有切片均保持 `visible = true` 并参与空间布局；`frameFloat` 只负责把当前帧锚定在中心以及驱动各帧的距离透明度分级，不再裁切当前帧之前的历史切片。
- 滚轮 → 相机距离目标值 → 平滑缩放主体，距离范围为 3–15.5，并将镜头焦点对准当前 `frameFloat` 的连续时间切片中心；使用更快的可打断阻尼，不修改 `frameFloat`、`yaw`、`pitch` 或 `spread`。
- `offset = index - frameFloat` → 当前帧中心锚定的深度位置；`slice.position = (0, 0, -offset × 0.62 × spread)`，切片自身旋转保持为 0。
