...
- 播放时间推进到第 N 帧后，`index < floor(frameFloat)` 的历史切片直接设为不可见；当前帧和未播放帧仍保留在时间隧道中，避免已越过的切片继续从镜头前方堆叠。
...
- 同次修改将滚轮职责限定为主体缩放：向上滚轮拉近相机、向下滚轮拉远相机，使用 5.2–15.5 的距离边界和可打断阻尼，不改变时间帧或旋转角度；放大时相机对准当前时间切片的连续中心，页面改为单屏舞台，不再用滚动驱动时间。（后续最近距离已调整为 3。）
