...
- 播放时当前帧锚定在中央焦点，所有可见切片保持正面平行，并沿卡片法线的世界 Z 轴以 `0.42` 的固定步进分层；每张切片都是独立平面，彼此有真实深度间隔，不再共面、相交或形成 X+Z 曲线。
- 播放推进到第 N 帧后，索引小于 `floor(frameFloat)` 的历史切片会从渲染中隐藏，视为已越过相机视野；当前帧与尚未播放的切片继续组成平行深度序列，正面形成嵌套层次，侧面可见分开的卡片边缘。
