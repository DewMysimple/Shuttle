# 蝴蝶 · 3D 时间切片

这是一个独立的 Three.js/Vite 交互作品页。当前时间切片源自：

`C:\Users\Administrator\Desktop\Free\BUTTERFLY_FLAP_FAST_FOLLOW_PATH_1.blend`

工程记忆：快速交接看 [`wiki_memory/README.md`](./wiki_memory/README.md)，完整历史兼容记录看 [`PROJECT_MEMORY.md`](./PROJECT_MEMORY.md)。结构化记忆的维护协议位于 [`wiki_memory/AGENTS.md`](./wiki_memory/AGENTS.md)。

## 生成网页素材

在 PowerShell 中执行蝴蝶时间切片导出：

```powershell
& 'F:\Blender\blender.exe' --background 'C:\Users\Administrator\Desktop\Free\BUTTERFLY_FLAP_FAST_FOLLOW_PATH_1.blend' --python '.\scripts\export_butterfly_slices.py' -- "$PWD\public\assets"
```

蝴蝶文件的动画范围为 1–91 帧，包含翅膀拍动和沿路径运动。导出脚本只使用带“展示_”前缀的蝴蝶网格，隐藏重复源对象和展示台；它不使用 `.blend` 原相机，而是为每一帧创建一个沿蝴蝶动画节点本地正面轴跟随的临时相机，保持蝴蝶正面并输出 91 张 768×768 RGBA PNG。PNG 中保留每一帧的翅膀姿态，网页再把它们沿 Z 轴排列成时间切片。

原场景中的相机、地面和重复对象不会写入网页运行资产，也不会被修改；每张图使用透明背景，便于网页在粉彩舞台上继续叠加光晕和纵深切片。

网页运行时先读取 `slice-manifest.json`，再按清单中的数量加载 PNG；因此切片数量不再写死为 76。进入页面后时间切片默认以中等展开状态显示，不会在中央额外立起一个实时模型。

源 `.blend` 只作为离线采样输入，不会被复制进网页仓库。

## 启动

```powershell
npm install
npm run dev
```

然后打开 Vite 输出的本地地址。

也可以直接双击 `runStart.cmd` 启动；脚本会自动切换到工程目录，检查 Node.js/npm，并在依赖缺失时执行 `npm install`。

## 交互

- 长按画布约 260ms 后播放 manifest 中的全部蝴蝶时间状态：左键长按正向播放，右键长按倒放，松开立即暂停；播放到对应边界后停留，再次长按会从相应起点重新播放。
- 画布右侧的“正放”和“倒放”按钮单击即可开始对应方向播放，再次点击当前播放方向可暂停；“开头”和“结尾”按钮单击后会暂停并直接跳到首帧或尾帧。
- 页面只显示 manifest 中的透明时间切片，不加载中心实时 GLB 模型。
- 播放时固定时间轴只负责空间结构：91 张切片以时间中点为固定中心，沿卡片法线的世界 Z 轴以 `1` 的固定步进分层，`frameFloat` 不再推动整组切片高速前后扫动；相邻切片保持正面平行，不共面、不相交，也不形成 X+Z 曲线。固定隧道在播放中仍保持低透明度可读，`playbackCursor` 会沿其中的 Z 轴时间位置连续穿过，补回纵深运动。
- 独立的 `focusLayer` 始终位于视觉中心，用当前帧和下一帧纹理按 `frameFloat` 小数部分连续交叉淡化；主焦点约为 `0.99` 不透明度，保留蝴蝶原始蓝色，仅使用低强度暖色偏移、轻微放大和蓝色局部微光。播放中最多保留 5 层低透明度历史/未来残影，残影同时具有横向与 Z 轴间距；卡片结构、时间游标和环境 halo 只做分层增强，避免形成规则矩形墙或白雾。
- 播放暂停后残影会平滑退出，固定时间轴恢复普通展示；收拢状态仍由主焦点层显示当前 `frameFloat`，不会出现空白。`prefers-reduced-motion` 下关闭残影漂移和微光呼吸，但保留稳定的主帧高亮与帧间交叉淡化。
- 画布拖拽只控制视角：横向拖拽连续累积方位角，可绕蝴蝶完整旋转 360° 及更多圈，每个视口宽度约旋转 396°；向下拖动是俯视，向上拖动是仰视，纵向范围为 ±90°，并提高了俯仰响应。
- 旋转采用更快的可打断阻尼跟随（拖拽/释放为 18/10），松手后只做短暂、无惯性的角度收敛，不会继续甩动。
- 约 8px 只作为起拖死区；超过死区后，水平位移独立控制 yaw、垂直位移独立控制 pitch，斜向拖动会同时改变两个角度；正向或倒放期间仍可继续拖拽旋转视角，松开才暂停播放。
- 相机固定，时间切片作为一个转台统一旋转；滚轮只控制相机距离（3–15.5），向上放大、向下缩小，缩放响应已加快，并以视觉中心的主焦点为聚焦中心。相机观察高度使用 `CAMERA_FOCUS_Y = 0.04`，最大放大时主体不会持续落在画面下方，不改变时间或旋转角度。
- “展开切片”按钮独立控制时间轴，不会改变当前视角；按 `R` 可恢复默认视角。
- 键盘聚焦画布后，按住空格正向播放、松开暂停；左右方向键旋转 10°，上下方向键调整 4° 俯仰，`Home` / `End` 控制收拢和展开。
- 时间切片组保留整体呼吸、粒子和轨道光环等环境动效，但不会再对单张卡片施加位置或角度扰动；系统开启 reduced-motion 时会自动降低动态幅度。
