...
- 长按画布约 260ms 后播放 manifest 中的全部兰花时间状态，松开立即暂停；播放到末帧后停在末帧，再次长按会从首帧重新播放。
- 页面只显示 manifest 中的透明时间切片，不加载中心实时 GLB 模型。
- 播放时当前帧锚定在中央焦点，前后切片沿 X+Z 轴穿过镜头，正面形成时间隧道，侧面保留独立间距；相邻切片步进为 X 0.16、Z 0.30。
- 画布拖拽只控制视角：横向拖拽连续累积方位角，可绕兰花完整旋转 360° 及更多圈；向下拖动是俯视，向上拖动是仰视，纵向范围限制在 ±28°。
- 旋转采用可打断的柔和阻尼跟随，松手后只做短暂、无惯性的角度收敛，不会继续甩动。
- 约 8px 只作为起拖死区；超过死区后，水平位移独立控制 yaw、垂直位移独立控制 pitch，斜向拖动会同时改变两个角度。
- 相机固定，时间切片作为一个转台统一旋转；滚轮只控制相机距离，向上放大、向下缩小，不改变时间或旋转角度。
- “展开切片”按钮独立控制时间轴，不会改变当前视角；按 `R` 可恢复默认视角。
- 键盘聚焦画布后，按住空格播放、松开暂停；左右方向键旋转 10°，上下方向键调整 4° 俯仰，`Home` / `End` 控制收拢和展开。
