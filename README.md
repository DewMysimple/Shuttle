...
- 播放时当前帧锚定在中央焦点，全部切片保持正面平行，并沿卡片法线的世界 Z 轴以 `0.62` 的固定步进分层；每张切片都是独立平面，彼此有真实深度间隔，不再共面、相交或形成 X+Z 曲线。
- 全部时间切片始终参与渲染：当前帧之前的历史切片不会被删除或隐藏，只按与当前帧的距离分级淡化；当前帧与前后时间状态共同组成完整的平行深度序列，正面形成嵌套层次，侧面可见分开的卡片边缘。
- 画布拖拽只控制视角：横向拖拽连续累积方位角，可绕兰花完整旋转 360° 及更多圈，每个视口宽度约旋转 333°；向下拖动是俯视，向上拖动是仰视，纵向范围扩大到 ±90°，并提高了俯仰响应。
