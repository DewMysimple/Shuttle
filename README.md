...
- 播放推进到第 N 帧后，索引小于 `floor(frameFloat)` 的历史切片会从渲染中隐藏，视为已越过相机视野；当前帧与尚未播放的切片继续组成前方时间轨迹。
- 画布拖拽只控制视角：横向拖拽连续累积方位角，可绕兰花完整旋转 360° 及更多圈，每个视口宽度约旋转 288°；向下拖动是俯视，向上拖动是仰视，纵向范围限制在 ±36°，并提高了俯仰响应。
...
- 相机固定，时间切片作为一个转台统一旋转；滚轮只控制相机距离（5.2–15.5），向上放大、向下缩小，并以当前时间切片为聚焦中心，不改变时间或旋转角度。
