...
- 全部时间切片始终参与渲染：当前帧之前的历史切片不会被删除或隐藏，只按与当前帧的距离分级淡化；当前帧图像约为 `0.96` 不透明度，相邻帧约为 `0.16–0.30`，中距离帧约为 `0.06–0.14`，远距离帧约为 `0.02–0.06`，从而避免所有兰花切片叠加成白雾。
- 当前时间切片同时使用最高色彩分级和独立的暖粉色加法微光；微光只跟随当前时间焦点连续过渡，并以约 `1.7s` 周期轻微呼吸，播放时不会在整数帧之间闪烁。收拢切片时以当前 `frameFloat` 四舍五入后的时间切片为唯一清晰焦点，其余重叠帧淡出；重新展开时再恢复完整时间轨迹。
