...
- 播放时固定时间轴只负责空间结构：166 张切片以时间中点为固定中心，沿卡片法线的世界 Z 轴以 `1` 的固定步进分层，`frameFloat` 不再推动整组切片高速前后扫动；相邻切片保持正面平行，不共面、不相交，也不形成 X+Z 曲线。
- 独立的 `focusLayer` 始终位于视觉中心，用当前帧和下一帧纹理按 `frameFloat` 小数部分连续交叉淡化；主焦点约为 `0.99` 不透明度，使用最高约 `1.90` 饱和度、`1.29` 对比度、暖珊瑚偏移、轻微放大和局部微光。播放中最多保留 5 层低透明度历史/未来残影，卡片结构和远端图像同步压低，避免形成规则矩形墙或白雾。
